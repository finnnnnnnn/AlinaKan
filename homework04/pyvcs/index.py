...
class GitIndexEntry(tp.NamedTuple):
    ...
    def unpack(data: bytes) -> "GitIndexEntry":
        ...
        unpack = list(struct.unpack(f, data))
        unpack[-1] = unpack[-1][: len(unpack[12]) - 3].decode()
...
def read_index(gitdir: pathlib.Path) -> tp.List[GitIndexEntry]:
    ...
    for i in range(len_data):
        need = b"\x00\x00\x00"
        end = data[a + 62:].find(need) + a + 62 + 3
        ans.append(GitIndexEntry.unpack(data[a:end]))
        a = end
    return ans
...
def write_index(gitdir: pathlib.Path, entries: tp.List[GitIndexEntry]) -> None:
    with open(gitdir / "index", "wb") as f:
        ...
        hash_1 = struct.pack("!4s2i", *values)
        f.write(hash_1)
        for element in entries:
            hash_1 += element.pack()
            f.write(element.pack())
        hash2 = hashlib.sha1(hash_1).hexdigest()
